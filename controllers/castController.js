    ...
    for (const [index, cast] of casts.entries()) {
      ...
      if (existingCast) {
        ...
        if (existingAssociation) {
          ...
          // Update the role in castsOnMovies if it's different
          await prisma.castsOnMovies.updateMany({
            where: {
              castId,
              movieId: movie.id,
            },
          ...
          addedCasts.push({
            ...
            message: `Cast '${name}' was already associated with this movie, role updated.`,
          });

          continue; // Move to the next cast
        }
      } else {
